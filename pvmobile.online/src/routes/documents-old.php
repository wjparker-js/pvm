...
$app->get('/api/documents/{apikey}/{user}/{pid}/{search}', function (Request $request, Response $response, $args) {
        ...
        if(substr($searchCondition,0,5) == "docno"){
            ...
            $searchCond .= "DocumentNumber like '%".$searchCondition."%' or ";
            $searchCond .= "ReasonName like     '%".$searchCondition."%' or ";
            ...
            $searchCond .= "DocumentNumber like '%".$searchCondition."%' or ";
            $searchCond .= "Rev like            '%".$searchCondition."%' or ";
            $searchCond .= "Title like          '%".$searchCondition."%' or ";
            $searchCond .= "IssueDate like      '%".$searchCondition."%' or ";
            $searchCond .= "RegDate like        '%".$searchCondition."%' or ";
            $searchCond .= "FileExtension like  '%".$searchCondition."%' or ";
            $searchCond .= "CurrentDoc like     '%".$searchCondition."%') ";
        }
...
/*
        $sql = "SELECT Folderid FROM FolderAccess where ProjectRoleID = '".$ProjectRoleID."'  ORDER BY Folderid";
...
*/
        ...
        $sql .= "WHERE    ";
        $sql .= "[ProjectFolders".$ProjectID."].FolderID in (SELECT FolderID FROM FolderAccess WHERE (ProjectRoleID = '$ProjectRoleID')) ";
        if($allRevs == "no"){
        $sql .= "AND ";
        ...
        $sql = "SELECT * from ".$tempFile." where ".$searchCond." ORDER BY RegDate DESC, Rev DESC";
...
*/        
        ...
        //return '"data":{"inserts":{"technologies":'.$docsj.'}}';       
