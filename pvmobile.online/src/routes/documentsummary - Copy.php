...
$app->get('/api/documentsummary/{apikey}/{user}/{pid}/{days}', function (Request $request, Response $response, $args) {
        ...
        $time = time();
        $date = date("Y-m-d", mktime(0,0,0,date("n", $time),date("j",$time) -$days ,date("Y", $time)));

        $sql .= "AND ";
        //$sql .= "(Regdate > '".$date."') ";
        $sql .= "(IssueDate > CAST(GETDATE() - ".$days." AS DATE)) ";
        
        $sql .= "AND "; 
        $sql .= "([ProjectIndex".$ProjectID."].FileStatus = 1)  ";
        $sql .= "AND ";
        $sql .= "([ProjectIndex".$ProjectID."].FolderID IS NOT NULL) order by [ProjectIndex".$ProjectID."].DateCreated desc ";
