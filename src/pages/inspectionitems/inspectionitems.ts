...
export class InspectionitemsPage {
  ...
  ionViewWillEnter(): void {
    ...
    if (this.temp == 0 && this.who == "sub") {
      this.temp = "sub";
    }
    if (this.who == "subi") {
      this.temp = "subi";
    }
    if (this.temp == 1 && this.who == "ins") {
      this.temp = "ins";
    }
    if (this.lid1 == undefined) {
      this.lid1 = "NULL"
    }
    if (this.lid2 == undefined) {
      this.lid2 = "NULL"
    }
    if (this.lid3 == undefined) {
      this.lid3 = "NULL"
    }
    if (this.lid4 == undefined) {
      this.lid4 = "NULL"
    }
  ...
  sendPhotoData(itemid, template, number, click, noteno) {
    ...
    this.http.post(upurl, formData).map(res => res.json()).subscribe(data => {
      ...
      console.log("Error Uploading sendPhotoData error.")
    });
  ...
  sendReport() {
    ...
    if (this.frmData1.notes == '') {
      this.frmData1.notes = "No notes";
    }
    ...
    if (this.selectedlocation != "") {
      ...
      if (this.showlocationlookup == 0) {
        ...
        this.http.get(formurl).map(res => res.json()).subscribe(data => {
          ...
          err => {
            console.log("Error sending subby sendReport data");
          }
        );
      }
      ...
      if (this.showlocationlookup == 1) {
        ...
        this.http.get(formurl1).map(res => res.json()).subscribe(data => {
          ...
          err => {
            console.log("Error sending inspector sendReport data");
          }
        );
      }
